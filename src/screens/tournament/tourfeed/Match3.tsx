...
const TourFeedMatch3: React.FC<{ tournament: any }> = ({ tournament }) => {
  // Use final matchup for this screen
  const currentMatchup = tournament.rounds?.final;
  ...
  const getPlayerNamesForMatch = (matchIndex: number, teamIndex: number) => {
    ...
    switch (matchIndex) {
      case 0:
        return players.map((p: Player) => p.name).join(", ");
      case 1:
        ...
        {!team1 || !team2 ? (
          <View style={styles.noTeamsMessage}>
            ...
            <Text style={styles.noTeamsText}>
              Winners must be decided to proceed to the final.
            </Text>
          </View>
        ) : (
          <FlatList
            ...
            renderItem={({ item: match, index }) => {
              ...
              return (
                <MatchCard
                  ...
                  teamStartIndex={-1} // Special value for Final
                  isReadOnly={true}
                />
              );
            }}
